'''Redis Protocol Parser, from aio-redis
...
class PyReader:
    ...
    def feed(self, data, o: int = 0, c: int = -1):
        """Feed data to parser."""
        if c == -1:
            c = len(data) - o
        if o < 0 or c < 0:
        ...
        self._parser.buf.extend(data[o : o + c])  # noqa E203
...
class Parser:
    ...
    def readone(self):
        if not self.buf[self.pos : self.pos + 1]:  # noqa E203
            yield from self.waitany()
        val = self.buf[self.pos : self.pos + 1]  # noqa E203
        self.pos += 1
        return val
    ...
    def readline(self, size: Optional[int] = None):
        if size is not None:
            ...
            while offset < 0:
                yield from self.waitany()
                offset = self.buf.find(b'\r\n', self.pos)
        val = self.buf[self.pos : offset]  # noqa E203
        self.pos = 0
